...
export default function Study() {
      ...
      try {
        ...
        setCards(
          (current) =>
            (current = {
              ...current,
              stack: response.cards,
              deckLength: response.cards.length,
            })
        );
      } catch (error) {
        if (error === "AbortError") {
  ...
  const handleFlip = () => {
    if (cards.side === "front") {
      setCards(
        (current) =>
          (current = { ...current, flip: !current["flip"], side: "back" })
      );
    } else {
      setCards(
        (current) =>
          (current = { ...current, flip: !current["flip"], side: "front" })
      );
    }
  };
  const handleNext = () => {
    if (cards.currentCard < cards.deckLength) {
      ...
      if (cards.currentCard === cards.deckLength - 1 && cards.flip) {
        const message = "Click 'OK' to restart deck. Click 'Cancel' to return to the homepage.";
  ...
  if (cards.deckLength < 3) {
    return (
      <>
        <BreadCrumbNav string="Study" deck={deck} />
        <div>
          ...
          <div className="card">
            <div className="card-body">
              ...
              <p className="card-text">
                You need at least 3 cards to study. There are {cards.deckLength}{" "}
                in this deck.
              </p>
